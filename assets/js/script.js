...
function getSkills() {
  return fetch('./skills.json').then(function (r) {
...
function showSkills(skills) {
  ...
  // These cards didn't exist when app.js set up ScrollReveal, so they need
  // their own reveal call now that they're actually in the DOM.
  if (window.sr) window.sr.reveal('.skills .bar', { interval: 30 });
}
...
function showWorkPreview(projects) {
  ...
  if (window.sr) window.sr.reveal('.work .box', { interval: 150 });
}
getProjects().then(showWorkPreview).catch(function (err) { console.error('Could not load projects.json', err); });

/* ===== LINKEDIN RECOMMENDATIONS — rendered from recommendations.json =====
   To add/remove a recommendation: drop the screenshot in assets/images/recommendations/
   and add/remove its filename in recommendations.json. Nothing else needs to change. */
function getRecommendations() {
  return fetch('./recommendations.json').then(function (r) {
    if (!r.ok) throw new Error('recommendations.json HTTP ' + r.status);
    return r.json();
  });
}
function showRecommendations(items) {
  ...
  if (!items || !items.length) {
    container.innerHTML = '<p class="rec-empty">No recommendations added yet.</p>';
    return;
  }
  var html = '';
  items.forEach(function (item) {
    html += '' +
      '<figure class="rec-card">' +
        '<img loading="lazy" draggable="false" src="./assets/images/recommendations/' + item.file + '" alt="' + (item.alt || 'LinkedIn recommendation') + '">' +
      '</figure>';
  });
  container.innerHTML = html;
  if (window.sr) window.sr.reveal('.recommendations .rec-card', { interval: 150 });
}
getRecommendations().then(showRecommendations).catch(function (err) { console.error('Could not load recommendations.json', err); });
